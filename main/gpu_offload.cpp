...
#pragma omp parallel for schedule(static)
  ...
  while (t < data.T) {
    ...
    if (it > 0)
      dt = 0.2 * data.hx / (1e-2 + cel); // to avoid cell-crossing
    ...
    my_timer.tic("build_colors");
    {
      ...
      color_offsets[0] = 0;
      for (int c = 0; c < 4; c++)
        color_offsets[c + 1] = color_offsets[c] + counts[c];

      int pos[4] = {color_offsets[0], color_offsets[1], color_offsets[2],
                    color_offsets[3]};
      for (int ip = 0; ip < np; ip++) {
        int cell = d_p2g[ip];
        int r = cell % nrows;
        int c = cell / nrows;
        int col = (r % 2) * 2 + (c % 2);
        color_indices[pos[col]++] = ip; // array with clusterized particles
      }
    }
...
#pragma omp target data map(to : d_p2g[0 : np], d_Mp[0 : np], d_cidx[0 : np],  \
...
#pragma omp target teams distribute parallel for
        for (int ip = 0; ip < np; ip++) {
          ...
          d_Fb_x[ip] =
              h > 1e-3
                  ? -0.0 * 2.5 *
                        (rho * h * g_c * tan_fa + rho * g_c * vx * vx / 100.0) *
                        vx / (nv + 0.001)
                  : 0.0;
          d_Fb_y[ip] =
              h > 1e-3
                  ? -0.0 * 2.5 *
                        (rho * h * g_c * tan_fa + rho * g_c * vy * vy / 100.0) *
                        vy / (nv + 0.001)
                  : 0.0;
        }
