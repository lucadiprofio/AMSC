...
int main() {
  ...
  #pragma omp target data \
    map(to: d_Z[0:nn], d_dZdx[0:nn], d_dZdy[0:nn], d_Mp[0:np]) \
    map(tofrom: d_x[0:np], d_y[0:np], d_vpx[0:np], d_vpy[0:np], \
        d_apx[0:np], d_apy[0:np], d_hp[0:np], d_Ap[0:np], d_Vp[0:np], \
        d_mom_px[0:np], d_mom_py[0:np], d_vpx_dx[0:np], d_vpx_dy[0:np], \
        d_vpy_dx[0:np], d_vpy_dy[0:np], d_Fb_x[0:np], d_Fb_y[0:np], \
        d_Fric_px[0:np], d_Fric_py[0:np], d_F11[0:np], d_F12[0:np], \
        d_F21[0:np], d_F22[0:np], d_dZxp[0:np], d_dZyp[0:np], d_Zp[0:np], \
        d_hpZ[0:np], d_Fpx[0:np], d_Fpy[0:np], d_vpxL[0:np], d_vpyL[0:np], d_H[0:np], \
        d_Mv[0:nn], d_mom_vx[0:nn], d_mom_vy[0:nn], d_F_ext_vx[0:nn], d_F_ext_vy[0:nn], \
        d_F_int_vx[0:nn], d_F_int_vy[0:nn], d_Ftot_vx[0:nn], d_Ftot_vy[0:nn], \
        d_avx[0:nn], d_avy[0:nn], d_vvx[0:nn], d_vvy[0:nn], d_Fric_x[0:nn], d_Fric_y[0:nn], \
        d_FPxv[0:nn], d_FPyv[0:nn], d_vvxL[0:nn], d_vvyL[0:nn], d_HV[0:nn], \
        d_pv_rho[0:nn], d_pv_vvx[0:nn], d_pv_vvy[0:nn], d_pv_avx[0:nn], d_pv_avy[0:nn])
        {
          ...
          while (fixed ? (it < data.NSTEPS) : (t < data.T)) {
            ...
            my_timer.tic("gpu_block");
              ...
              // (1) azzera gli accumulatori nodali SUL DEVICE (step 0)
              #pragma omp target teams distribute parallel for
              for (int iv = 0; iv < nn; iv++) {
                d_Mv[iv]=0; d_mom_vx[iv]=0; d_mom_vy[iv]=0;
                d_F_ext_vx[iv]=0; d_F_ext_vy[iv]=0; d_F_int_vx[iv]=0; d_F_int_vy[iv]=0;
                d_Ftot_vx[iv]=0; d_Ftot_vy[iv]=0; d_avx[iv]=0; d_avy[iv]=0;
                d_vvx[iv]=0; d_vvy[iv]=0; d_Fric_x[iv]=0; d_Fric_y[iv]=0;
                d_FPxv[iv]=0; d_FPyv[iv]=0; d_vvxL[iv]=0; d_vvyL[iv]=0; d_HV[iv]=0;
              ...
              // (2) connettività rimappata ogni step (solo 4 array piccoli)
              #pragma omp target data map(to: d_p2g[0:np], d_cstart[0:ncells+1], \
                                              d_cptcls[0:np], d_ccidx[0:ncells])
              {
            ...
            my_timer.tic("build_colors");
              ...
              int cpos[4] = {color_cell_offsets[0], color_cell_offsets[1], color_cell_offsets[2], color_cell_offsets[3]};
