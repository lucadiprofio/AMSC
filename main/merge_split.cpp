...
int main() {
  ...
  for (idx_t ip = 0; ip < num_particles; ++ip) {
    ptcls.dprops["F_11"][ip] =
        -0.5 * data.rho * data.g * (ptcls.dprops["hp"][ip]);
    ptcls.dprops["F_12"][ip] = 0.0;
    ptcls.dprops["F_21"][ip] = 0.0;
    ptcls.dprops["F_22"][ip] =
        -0.5 * data.rho * data.g * (ptcls.dprops["hp"][ip]);
  }
  ...
  while (t < data.T) // data.T;
  {
    ...
    /*      double max_vel_x = *std::max_element(ptcls.dprops["vpx"].begin(),
       ptcls.dprops["vpx"].end()); double max_vel_y =
       *std::max_element(ptcls.dprops["vpy"].begin(),
       ptcls.dprops["vpy"].end()); double hmean =
       accumulate(ptcls.dprops["hp"].begin(), ptcls.dprops["hp"].end(),
       0.0/ptcls.dprops["hp"].size()); double max_vel =
       std::max(std::sqrt(data.g * hmean)+max_vel_x,-std::sqrt(data.g *
       hmean)+max_vel_y);
          //  double max_vel = std::max(1+max_vel_x,1+max_vel_y);
            cel = std::abs(max_vel); */

    double max_vel_x = *std::max_element(ptcls.dprops["vpx"].begin(),
                                         ptcls.dprops["vpx"].end());
    double min_vel_x = *std::min_element(ptcls.dprops["vpx"].begin(),
                                         ptcls.dprops["vpx"].end());

    max_vel_x = std::max(std::abs(max_vel_x), std::abs(min_vel_x));

    double max_vel_y = *std::max_element(ptcls.dprops["vpy"].begin(),
                                         ptcls.dprops["vpy"].end());
    double min_vel_y = *std::min_element(ptcls.dprops["vpy"].begin(),
                                         ptcls.dprops["vpy"].end());

    max_vel_y = std::max(std::abs(max_vel_y), std::abs(min_vel_y));

    double hmean =
        *std::max_element(ptcls.dprops["hp"].begin(), ptcls.dprops["hp"].end());
    double max_vel = std::max(std::sqrt(data.g * hmean) + max_vel_x,
                              std::sqrt(data.g * hmean) + max_vel_y);
    cel = std::abs(max_vel);

    if (it > 0)
      dt = 0.2 * data.hx /
           (1e-2 + cel); // 0.7      0.2 *  data.hx / (1e-4 + cel);
    ...
    for (auto icell = grid.begin_cell_sweep(); icell != grid.end_cell_sweep();
      ...
      for (auto inode = 0;
        ...
        if (ptcls.grd_to_ptcl.count(icell->get_global_cell_idx()) > 0)
          for (auto ip = 0;
               ip < ptcls.grd_to_ptcl.at(icell->get_global_cell_idx()).size();
               ++ip) {
            auto gp = ptcls.grd_to_ptcl.at(icell->get_global_cell_idx())[ip];
            ptcls.dprops["Fric_px"][gp] =
                0.0 * ptcls.dprops["Ap"][gp] *
                ptcls.dprops["Fb_x"][gp]; // - ptcls.dprops["Mp"][gp] *    9.81
                                          // * ptcls.dprops["dZxp"][gp] +
                                          // ptcls.dprops["Ap"][gp] *
                                          // ptcls.dprops["Fb_x"][gp]  ;
            ptcls.dprops["Fric_py"][gp] =
                0.0 * ptcls.dprops["Ap"][gp] *
                ptcls.dprops["Fb_y"][gp]; //  - ptcls.dprops["Mp"][gp] *    9.81
                                          //  * ptcls.dprops["dZyp"][gp]  +
                                          //  ptcls.dprops["Ap"][gp] *
                                          //  ptcls.dprops["Fb_y"][gp]  ;
          }
      }
    }
    ...
    for (auto icell = grid.begin_cell_sweep(); icell != grid.end_cell_sweep();
      ...
      for (auto inode = 0;
        ...
        vars["F_ext_vx"][gv] =
            -vars["Mv"][gv] * 9.81 * 1.0 * vars["dZdx"][gv] +
            vars["Fric_x"]
                [gv]; // vars["dZdx"][gv]; // vars["dZdx"][gv]; //  dZdx[gv]  ;
                      // //-  ptcls.dprops["Ap"][gp] * ptcls.dprops["Fb_x"][gp];

        vars["F_ext_vy"][gv] =
            -vars["Mv"][gv] * 9.81 * 1.0 * vars["dZdy"][gv] +
            vars["Fric_y"]
                [gv]; // vars["dZdy"][gv]; //  -   ptcls.dprops["Ap"][gp] *
                      // ptcls.dprops["Fb_y"][gp];
      }
    }
    ...
    ptcls.p2gd(vars, std::vector<std::string>{"F_11", "F_21"},
               std::vector<std::string>{"F_12", "F_22"}, "Vp",
               std::vector<std::string>{"F_int_vx", "F_int_vy"});
    ...
    for (auto icell = grid.begin_cell_sweep(); icell != grid.end_cell_sweep();
      ...
      for (auto inode = 0;
        ...
        auto iv = icell->gt(inode);
        vars["avx"][iv] =
            vars["Mv"][iv] > 1e-8 ? vars["Ftot_vx"][iv] / vars["Mv"][iv] : 0.0;
        vars["avy"][iv] =
            vars["Mv"][iv] > 1e-8 ? vars["Ftot_vy"][iv] / vars["Mv"][iv] : 0.0;

        vars["vvx"][iv] =
            vars["Mv"][iv] > 1e-8 ? vars["mom_vx"][iv] / vars["Mv"][iv] : 0.0;
        vars["vvy"][iv] =
            vars["Mv"][iv] > 1e-8 ? vars["mom_vy"][iv] / vars["Mv"][iv] : 0.0;
      }
    }
    my_timer.toc("step 4");
  ...
  ptcls.dprops.at("vpy_dy").assign(ptcls.num_particles, 0.0);
    ...
    for (auto icell = grid.begin_cell_sweep(); icell != grid.end_cell_sweep();
      ...
      for (auto inode = 0;
        ...
        if (ptcls.grd_to_ptcl.count(icell->get_global_cell_idx()) > 0)
          for (auto gp = 0;
               gp < ptcls.grd_to_ptcl.at(icell->get_global_cell_idx()).size();
               ++gp) {
            auto ip = ptcls.grd_to_ptcl.at(icell->get_global_cell_idx())[gp];
            ptcls.dprops["hp"][ip] /= (1 + dt * (ptcls.dprops["vpx_dx"][ip] +
                                                 ptcls.dprops["vpy_dy"][ip]));
            ptcls.dprops["mom_px"][ip] =
                ptcls.dprops["vpx"][ip] * ptcls.dprops["Mp"][ip];
            ptcls.dprops["mom_py"][ip] =
                ptcls.dprops["vpy"][ip] * ptcls.dprops["Mp"][ip];
            ptcls.dprops["Ap"][ip] /= (1 + dt * (ptcls.dprops["vpx_dx"][ip] +
                                                 ptcls.dprops["vpy_dy"][ip]));
            ptcls.dprops["Vp"][ip] =
                ptcls.dprops["hp"][ip] * ptcls.dprops["Ap"][ip];
            // ptcls.dprops["Ap"][ip] =  ptcls.dprops["hp"][ip] >= 1.e-2 ?
            // ptcls.dprops["Vp"][ip]/ptcls.dprops["hp"][ip] :
            // ptcls.dprops["hp"][ip]; //     (1+dt *
            // (ptcls.dprops["vpx_dx"][ip] + ptcls.dprops["vpy_dy"][ip])); //
            // ptcls.dprops["Vp"][ip]/ptcls.dprops["hp"][ip];
            // //ptcls.dprops["Mp"][ip] / (data.rho * ptcls.dprops["hp"][ip]);
          }
      }
    }
    ...
    for (idx_t ip = 0; ip < num_particles; ++ip) {
      norm_v[ip] = std::sqrt(ptcls.dprops["vpx"][ip] * ptcls.dprops["vpx"][ip] +
                             ptcls.dprops["vpy"][ip] * ptcls.dprops["vpy"][ip]);

      ptcls.dprops["Fb_x"][ip] =
          ptcls.dprops["hp"][ip] > 1.e-3
              ? -0.0 * 2.5 *
                    (data.rho * ptcls.dprops["hp"][ip] * data.g *
                         std::tan(fric_ang) +
                     data.rho * data.g * ptcls.dprops["vpx"][ip] *
                         ptcls.dprops["vpx"][ip] / 100.) *
                    ptcls.dprops["vpx"][ip] / (norm_v[ip] + 0.001)
              : 0.0;

      ptcls.dprops["Fb_y"][ip] =
          ptcls.dprops["hp"][ip] > 1.e-3
              ? -0.0 * 2.5 *
                    (data.rho * ptcls.dprops["hp"][ip] * data.g *
                         std::tan(fric_ang) +
                     data.rho * data.g * ptcls.dprops["vpy"][ip] *
                         ptcls.dprops["vpy"][ip] / 100.) *
                    ptcls.dprops["vpy"][ip] / (norm_v[ip] + 0.001)
              : 0.0;
    }
      ...
      for (auto inode = 0;
        ...
        if (ptcls.grd_to_ptcl.count(icell->get_global_cell_idx()) > 0)
          for (auto gp = 0;
            ...
            auto ip = ptcls.grd_to_ptcl.at(icell->get_global_cell_idx())[gp];
            norm_v[ip] =
                std::sqrt(ptcls.dprops["vpx"][ip] * ptcls.dprops["vpx"][ip] +
                          ptcls.dprops["vpy"][ip] * ptcls.dprops["vpy"][ip]);

            ALF[ip] = ptcls.dprops["hp"][ip] > 1.e-3
                          ? (6. * 50. * norm_v[ip]) /
                                ((ptcls.dprops["hp"][ip] + 0.001) * 2000.)
                          : 0.0;
            B[ip] = -114. / 32. - ALF[ip];
            Z1[ip] =
                (-B[ip] + std::sqrt(B[ip] * B[ip] - 4. * A * C)) / (2. * A);
            Z2[ip] =
                (-B[ip] - std::sqrt(B[ip] * B[ip] - 4. * A * C)) / (2. * A);
            ...
            s_xy[ip] =
                0.5 * (ptcls.dprops["vpx_dy"][ip] + ptcls.dprops["vpy_dx"][ip]);
            s_yy[ip] = ptcls.dprops["vpy_dy"][ip];

            D_xx[ip] = s_xx[ip];
            D_yx[ip] = s_xy[ip];
            D_zx[ip] = ptcls.dprops["hp"][ip] > 1.e-3
                           ? 0.5 * (3. / (2. + ZZ[ip])) *
                                 (ptcls.dprops["vpx"][ip] /
                                  (ptcls.dprops["hp"][ip] + 0.001))
                           : 0.0;

            D_xy[ip] = s_xy[ip];
            D_yy[ip] = s_yy[ip];
            D_zy[ip] = ptcls.dprops["hp"][ip] > 1.e-3
                           ? 0.5 * (3. / (2. + ZZ[ip])) *
                                 (ptcls.dprops["vpy"][ip] /
                                  (ptcls.dprops["hp"][ip] + 0.001))
                           : 0.0;

            D_xz[ip] = 0.0; // 0.0; // 0.5 * (3. / (2. + ZZ[ip])) *
                            // (ptcls.dprops["vpx"][ip] /
                            // (ptcls.dprops["hp"][ip] + 0.001) );
            D_yz[ip] = 0.0; // 0.0; // 0.5 * (3. / (2. + ZZ[ip])) *
                            // (ptcls.dprops["vpy"][ip] /
                            // (ptcls.dprops["hp"][ip]+ 0.001) );
            D_zz[ip] =
                -(ptcls.dprops["vpx_dx"][ip] + ptcls.dprops["vpy_dy"][ip]);

            invII[ip] = 0.5 * (D_xx[ip] * D_xx[ip] + D_yy[ip] * D_yy[ip] +
                               D_zz[ip] * D_zz[ip] + D_xz[ip] * D_xz[ip] +
                               D_yz[ip] * D_yz[ip] + D_xy[ip] * D_xy[ip]);

            sig_xx[ip] =
                invII[ip] != 0
                    ? (2000. / std::sqrt(invII[ip]) + 2. * 50.) * D_xx[ip]
                    : 0.0;
            sig_xy[ip] =
                invII[ip] != 0
                    ? (2000. / std::sqrt(invII[ip]) + 2. * 50.) * D_xy[ip]
                    : 0.0;
            sig_yy[ip] =
                invII[ip] != 0
                    ? (2000. / std::sqrt(invII[ip]) + 2. * 50.) * D_yy[ip]
                    : 0.0;
      ...
      for (auto inode = 0;
        ...
        if (ptcls.grd_to_ptcl.count(icell->get_global_cell_idx()) > 0)
          for (auto gp = 0;
            ...
            auto ip = ptcls.grd_to_ptcl.at(icell->get_global_cell_idx())[gp];
            ptcls.dprops["hpZ"][ip] =
                ptcls.dprops["hp"][ip] + ptcls.dprops["Zp"][ip];
          }
      }
    }
